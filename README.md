...
Nevertheless, identifying optimal portfolio by maximizing Sharpe ratio is only valid if asset returns are normally distributed. The Shapiro-Wilk test for the log returns of our assets reveal the fact that the return rates do not follow normal distribution. This paper is to suggest Roy’s Safety-First criterion as one of the alternatives that do not require a statistical distribution. Roy’s Safety-First criterion suggests that investors will choose the optimal portfolio that minimizing the probability which portfolio’s return fall below a safety rate called floor return (Francis & Kim, 2013)
